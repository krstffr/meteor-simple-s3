SimpleS3Class = function () {
					...
					if (!filterDontReturn) {
						if (key === 'Directory') {
							...
							if (bucketItem[key] === value && bucketItem.Filename) {
								// console.log('0:', bucketItem, value, bucketItem[key] === value );
								...
								if ( bucketItem.Filename ) {
									// console.log('1:', bucketItem, bucketItem[key], value, bucketItem[key] === value );
									...
									// console.log('2:', bucketItem, value, bucketItem[key] === value );
	...
	that.bucketList.get = function () {
		return Object.assign(
			{},
			Session.get('simple-s3__bucket-items'), {
			// Remove duplicates…
			Contents: _.uniq(Session.get('simple-s3__bucket-items').Contents, function(item) { 
				return item.Directory + item.Key;
			})
		});
	};
	...
	that.bucketList.addFolders = function () {

		var uniqueFolders = _.unique( _.map( that.bucketList.get().Contents, function(file) {
			return file.Key.replace(/[^\/]*$/, '');
		}) );

		var folders = _.filter( _.map( uniqueFolders, function(folder) {
			...
			return {
				...
				FileExtension: ".",
				Key: folder.replace(/[^\/]*$/, ''),
				SimpleS3ItemOnly: true
			};
		}), function( folder ) { return folder !== false } );

		// console.log(folders);

		that.bucketObject.Contents = that.bucketObject.Contents.concat( folders );
	...
	that.bucketList.prepareItems = function () {
		...
		// Add all folders as well
		that.bucketList.addFolders();
	...
	that.file.deleteFile = function ( key ) {
	...
	that.file.deleteFileMany = function ( items, index ) {
		that.ui.disable();
		...
		Meteor.call('simpleS3/fileDelete', items[index], function (err, res) {
			...
			if (items.length > index+1) {
				that.file.deleteFileMany( items, index+1 );
			} else {
				that.bucketList.updateFromAWS();
			}
		});
	...
	that.folder.deleteFolder = function ( key ) {
		...
		that.file.deleteFileMany( itemsToDelete, 0 );
	};
