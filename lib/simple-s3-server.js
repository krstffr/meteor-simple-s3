...
if (Meteor.settings.AWSAccessKeyId &&
	...
	// The settings for Slingshot uploads
	var slingshotSettings = {
	...
	// The user can choose to disable the cache header, but if not it will be set!
	if (!Meteor.settings.AWSDisableCacheHeader)
		slingshotSettings.cacheControl = 'public,max-age=2592000';

	// Setup Slingshot
	Slingshot.createDirective('simple-s3-uploads', Slingshot.S3Storage, slingshotSettings );
