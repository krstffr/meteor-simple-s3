...
if (process.env.AWS_BUCKET) {
	Meteor.settings.AWS.bucket = process.env.AWS_BUCKET;
	if (!Meteor.settings.AWS.public)
		Meteor.settings.AWS.public = { AWS: { bucket: false } };
...
if (!Meteor.settings.AWS.bucket)
	console.error('Missing Meteor.settings.AWS.bucket, can\'t setup Simple-S3');

if (Meteor.settings.AWS.accessKeyId &&
	Meteor.settings.AWS.secretAccessKey &&
	Meteor.settings.AWS.bucket) {
