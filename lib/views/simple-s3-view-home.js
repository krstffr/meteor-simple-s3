...
Template.simpleS3ViewHome.events({
	'click .simple-s3__move-file': function ( e ) {
		...
		var currentFileName = SimpleS3.getFileName( key );
		var newFileName = prompt('Select a new filename.', currentFileName);
		SimpleS3.file.rename( key, currentFileName, newFileName );
	},
	'click .simple-s3__get-url': function ( e ) {
		...
		window.prompt(
			"Copy the link below.",
			'http://'+Session.get('simple-s3__bucket-items').Name+'.s3.amazonaws.com/'+key
			);
	},
