...
Template.simpleS3ViewHome.helpers({
	...
	movingClasses: function () {
		...
		if (Session.get('simple-s3__files-movable') &&
			!Session.get('simple-s3__files-movable__current-moving-file'))
			return 'simple-s3__bucket-item--movable';

		if (Session.get('simple-s3__files-movable') &&
			Session.get('simple-s3__files-movable__current-moving-file') === this.Key)
			return 'simple-s3__bucket-item--to-be-moved';

		if (Session.get('simple-s3__files-movable') &&
			Session.get('simple-s3__files-movable__current-moving-file') &&
			this.Filename === '')
			return 'simple-s3__bucket-item--folder-is-taking-moving-files';
...
Template.simpleS3ViewHome.events({
	...
	'click .simple-s3__bucket-item--file.simple-s3__bucket-item--movable': function ( e ) {
		SimpleS3.file.selectFile( this.Key );
	},
